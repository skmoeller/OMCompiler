    ...
    case BackendDAE.EQUATION(exp=e1, scalar=e2, source=source, attr=eqAttr)
      algorithm
        ...
        try
          ...
          solveEqns := listReverse(solveEqns);
          solveCr := listReverse(solveCr);
          cr := if BackendVariable.isStateVar(var) then ComponentReference.crefPrefixDer(cr) else cr;
          source := ElementSource.addSymbolicTransformationSolve(true, source, cr, e1, e2, exp_, asserts);
          (eqSystlst, uniqueEqIndex) := List.mapFold(solveEqns, makeSolved_SES_SIMPLE_ASSIGN, uniqueEqIndex);

          (resEqs, uniqueEqIndex) := addAssertEqn(asserts, {SimCode.SES_SIMPLE_ASSIGN(uniqueEqIndex, cr, exp_, source, eqAttr)}, uniqueEqIndex+1);
          eqSystlst := listAppend(eqSystlst,resEqs);
          tempVars := createTempVarsforCrefs(List.map(solveCr, Expression.crefExp),tempVars);
        else
          Error.addInternalError("Not all cases are implemented in function createAllEquationOMSI", sourceInfo());
          fail();
        end try;
