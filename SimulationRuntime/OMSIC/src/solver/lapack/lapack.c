 ...
 */
omsi_status get_result(omsi_function_t* equationSystemFunc,
    ...
    for (i=0; i<equationSystemFunc->n_output_vars;i++) {
        ...
        equationSystemFunc->function_vars->reals[index] = lapack_data[i]
    }
    return omsi_ok;
}
...
/*
 * Copies necessary informations from equationSystemFunc into lapack_data
 * Input:   equationSystemFunc
 * Output:  lapack_data
 */
omsi_status set_lapack_data(omsi_function_t* equationSystemFunc,
                            DATA_LAPACK*     lapack_data) {
    ...
    /* allocate memory */
    lapack_data->A = (omsi_real*) omsi_allocateMemory(lapack_data->lda*lapack_data->n, sizeof(omsi_real));
    lapack_data->ipiv = (omsi_int*) omsi_allocateMemory(lapack_data->n, sizeof(omsi_int));
    lapack_data->B = (omsi_real*) omsi_allocateMemory(lapack_data->ldb*lapack_data->nrhs, sizeof(omsi_real));
 ...
 */
void set_lapack_a (DATA_LAPACK* lapack_data, omsi_function_t* equationSystemFunc) {
    ...
    for (i=0; i<lapack_data->lda; i++) {
        for (j=0; j<lapack_data->n; j++) {
            ...
            lapack_data->A[i*lapack_data->lda+j] = equationSystemFunc->function_vars->reals[i+j*lapack_data->n];
...
void set_lapack_b (DATA_LAPACK* lapack_data, omsi_function_t* equationSystemFunc) {
    ...
    for (i=0; i<lapack_data->lda; i++) {
        for (j=0; j<lapack_data->n; j++) {
            /* copy data from column-major to row-major style */
            lapack_data->B[i*lapack_data->lda+j] = equationSystemFunc->function_vars->reals[i+j*lapack_data->n];
            // ToDo: where exactly is this data stored in function_vars ????
        }
    }
}
...
int set_b_vector(sim_data_t* data, omsi_vector_t* vector){
