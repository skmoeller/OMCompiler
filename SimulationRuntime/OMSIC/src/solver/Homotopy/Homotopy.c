...
int solveSystemWithTotalPivotSearch(int n, double* x, double* A, int* indRow, int* indCol, int *pos, int *rank, int casualTearingSet)
{
   ...
   for (i = 0; i < n; i++) {
    ...
    if (absMax<DBL_EPSILON) {
      *rank = i;
      ...
      debugInt(LOG_NLS,"pomsition = ", *pos);
      break;
    }
    /* swap row indices */
    if (pRow!=i) {
  ...
  debugMatrixPermutedDouble(LOG_NLS_JAC,"Linear System Matrix [Jac res] after decompomsition",A, n, m, indRow, indCol);
  ...
  /* Return pomsition of largest value (1.0) */
  if (*pos<0) {
    *pos=indCol[n];
  }
...
int linearSolverWrapper(int n, double* x, double* A, int* indRow, int* indCol, int *pos, int *rank, int method, int casualTearingSet)
{
  ...
  switch(method){
    ...
    case 2: /* NLS_LS_LAPACK */
      ...
      debugMatrixDouble(LOG_NLS_JAC,"Linear system matrix [Jac res] after decompomsition:", A, n, n+1);
      debugDouble(LOG_NLS_JAC,"Determinant = ", detJac);
