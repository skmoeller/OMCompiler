...
//    rt_ext_tp_tick(&systemData->jacobianTimeClock);
    ...
    if(systemData->jacobianIndex != -1) {
      ...
      printf("analytical jacobian != -1, no Solution will be found");
    } else {
...
{ //solveNewton(DATA *data, threadData_t *threadData, int sysNumber);
...
//  DATA_NEWTON* solverData = (DATA_NEWTON*)(systemData->solverData);
  ...
  int nfunc_evals = 0;
  int continuous = 1;
  double local_tol = newtonData->ftol;
  eqSystemNumber = nonlinearSystem->equation_index;
...
//  relationsPreBackup = (modelica_boolean*) malloc(data->modelData->nRelations*sizeof(modelica_boolean));
  ...
  newtonData->calculate_jacobian = 0;
...
//    memcpy(newtonData->x, systemData->nlsx, newtonData->n*(sizeof(double)));
//  } else {
//    memcpy(newtonData->x, systemData->nlsxExtrapolation, newtonData->n*(sizeof(double)));
//  }
  nonlinearSystem->get_x(&omsiData->sim_data, newtonData->x);
  /* start solving loop */
  while(!giveUp && !success)
  {

    giveUp = 1;
//    newtonData->newtonStrategy = data->simulationInfo->newtonStrategy;
    _omc_newton(wrapper_fvec_newton, newtonData, omsiData);
...
//      printErrorEqSyst(IMPROPER_INPUT, modelInfoGetEquation(&data->modelData->modelDataXml,eqSystemNumber), data->localData[0]->timeValue);
    ...
    /* check for error  */
    xerror_scaled = enorm_(&newtonData->n, newtonData->fvecScaled);
    xerror = enorm_(&newtonData->n, newtonData->fvec);

    /* solution found */
    if((xerror <= local_tol || xerror_scaled <= local_tol) && newtonData->info > 0)
    {
      success = 1;
      nfunc_evals += newtonData->nfev;
...
//      }
      ...
      /* take the solution */
      memcpy(systemData->nlsx, newtonData->x, newtonData->n*(sizeof(double)));
...
//      infoStreamPrint(LOG_NLS, 0, "### No Solution for the casual tearing set at the first try! ###");
    ...
    {
      memcpy(newtonData->x, systemData->nlsxOld, newtonData->n*(sizeof(double)));
...
//      infoStreamPrint(LOG_NLS, 0, " - iteration making no progress:\t try old values.");
    ...
    {
      for(i = 0; i < newtonData->n; i++)
        newtonData->x->data[i] += systemData->nominal[i] * 0.01;
      retries++;
      giveUp = 0;
      nfunc_evals += newtonData->nfev;
//      infoStreamPrint(LOG_NLS, 0, " - iteration making no progress:\t vary solution point by 1%%.");
    ...
    {
      for(i = 0; i < newtonData->n; i++)
        newtonData->x->data[i] = systemData->nominal[i];
      retries++;
      giveUp = 0;
      nfunc_evals += newtonData->nfev;
//      infoStreamPrint(LOG_NLS, 0, " - iteration making no progress:\t try nominal values as initial solution.");
    ...
    else if(retries < 4  && data->simulationInfo->discreteCall)
    {
      ...
      memcpy(newtonData->x->data, systemData->nlsxOld, newtonData->n*(sizeof(double)));
      retries++;
...
//    messageClose(LOG_NLS);
  ...
  /* write statistics */
  systemData->numberOfFEval = newtonData->numberOfFunctionEvaluations;
  systemData->numberOfIterations = newtonData->numberOfIterations;
