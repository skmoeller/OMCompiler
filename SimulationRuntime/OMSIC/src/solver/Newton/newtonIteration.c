...
int allocateNewtonData(int size, void** voiddata)
{
  ...
  *voiddata = (void*)data;
  ...
  data->resScaling = _omsi_allocateVectorData(size);
  data->fvecScaled = _omsi_allocateVectorData(size);

  data->n = size;
  data->x = _omsi_allocateVectorData(size);
  data->fvec = _omsi_allocateVectorData(size);
  data->xtol = 1e-6;
  data->ftol = 1e-6;
  data->maxfev = size*100;
  data->epsfcn = DBL_EPSILON;
  data->fjac = _omsi_allocateVectorData(size);

  data->rwork = _omsi_allocateVectorData(size);
  data->iwork = (int*) malloc(size*sizeof(int));

  /* damped newton */
  data->x_new = _omsi_allocateVectorData(size);
  data->x_increment = _omsi_allocateVectorData(size);
  data->f_old = _omsi_allocateVectorData(size);
  data->fvec_minimum = _omsi_allocateVectorData(size);
  data->delta_f = _omsi_allocateVectorData(size);
  data->delta_x_vec = _omsi_allocateVectorData(size);
...
int freeNewtonData(void **voiddata)
{
  ...
  _omsi_deallocateVectorData(data->resScaling);
  _omsi_deallocateVectorData(data->fvecScaled);
  _omsi_deallocateVectorData(data->x);
  _omsi_deallocateVectorData(data->fvec);
  _omsi_deallocateVectorData(data->fjac);
  _omsi_deallocateVectorData(data->rwork);
  free(data->iwork);

  /* damped newton */
  _omsi_deallocateVectorData(data->x_new);
  _omsi_deallocateVectorData(data->x_increment);
  _omsi_deallocateVectorData(data->f_old);
  _omsi_deallocateVectorData(data->fvec_minimum);
  _omsi_deallocateVectorData(data->delta_f);
  _omsi_deallocateVectorData(data->delta_x_vec);
...
//  }
  ...
  *info = 1;
  ...
  _omsi_copyVector(solverData->f_old, fvec);

  error_f = current_fvec_enorm = enorm_(n, fvec);
  _omsi_copyVector(solverData->fvecScaled, solverData->fvec);
...
//    }
    ...
    {
      for (i =0; i<*n; i++)
        solverData->x_new->data[i]=x[i]-solverData->x_increment->data[i];
...
int solveLinearSystem(int* n, int* iwork, double* fvec, double *fjac, DATA_NEWTON* solverData)
{
  ...
  if (solverData->factorization == 0)
  {
    ...
    dgetrf_(n, n, fjac->data, n, iwork, &lapackinfo);
    solverData->factorization = 1;
    dgetrs_(&trans, n, &nrsh, fjac->data, n, iwork, fvec->data, n, &lapackinfo);
  }
  else
  {
    dgetrs_(&trans, n, &nrsh, fjac->data, n, iwork, fvec->data, n, &lapackinfo);
...
//    warningStreamPrint(LOG_NLS, 0, "illegal  input in argument %d", (int)lapackinfo);
  ...
  else
  {
    /* save solution of J*(x_{n+1} - x_n)=f */
	  _omsi_copyVector(solverData->x_increment, fvec);
  }
